...
xname =  (Redis:get(Fast.."Name:Bot") or "ابو ايان") 
...
YouCan = true
elseif tonumber(senderr) == tonumber(5099564264) then
YouCan = true
elseif tonumber(senderr) == tonumber(5099564264) then
YouCan = true
else
...
return send(msg_chat_id,msg_id,Reply_Status(rep_idd,"• هاذ مكانه وي لزراب😂 ").Reply,"md",true)  
else
Redis:sadd(Fast.."kholat:Group"..msg_chat_id,rep_idd) 
return send(msg_chat_id,msg_id,Reply_Status(rep_idd,"• تم ترقيته زربه من يسبح ارجع رتبته🙂👍🏻 ").Reply,"md",true)  
end
end
if TextMsg == "الحلوات" then
if ChannelJoin(msg) == false then
local chinfo = Redis:get(Fast.."ch:admin")
local reply_markup = bot.replyMarkup{type = 'inline',data = {{{text = 'اضغط للاشتراك', url = chinfo}, },}}
return send(msg.chat_id,msg.id,'*\n• عليك الاشتراك في قناة البوت لاستخذام الاوامر*',"md",false, false, false, false, reply_markup)
...
return send(msg_chat_id,msg_id,Reply_Status(rep_idd,"• هاي اجمل بنيه بل عالم  مح😂🙂 ").Reply,"md",true)  
else
Redis:sadd(Fast.."wtka:Group"..msg_chat_id,rep_idd) 
return send(msg_chat_id,msg_id,Reply_Status(rep_idd,"• غير تخبل وتستاهل ارفعهه وي لحلوات 🥺🫂  ").Reply,"md",true)  
end
end
if TextMsg == "متوحد" then
if ChannelJoin(msg) == false then
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"• ماكو جلاب هنا ارفعلي خل انطيهم عضم 🦴😂 , ","md",true)  
end
ListMembers = '\n*• قائمه الجلاب  \n ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉*\n'
for k, v in pairs(Info_Members) do
local UserInfo = bot.getUser(v)
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"• ماكو مطايه هنا😂😂 , ","md",true)  
end
ListMembers = '\n*• قائمه المطايه  \n ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉*\n'
for k, v in pairs(Info_Members) do
local UserInfo = bot.getUser(v)
...
local reply_markup = bot.replyMarkup{
type = 'inline',
data = {{{text = '-مسح المطايه ر', data = senderr..'/Delmar'},},}}
return send(msg_chat_id, msg_id, ListMembers, 'md', false, false, false, false, reply_markup)
end
if text == 'العار' then
if ChannelJoin(msg) == false then
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"• ماكو عارر هنا😂💔 , ","md",true)  
end
ListMembers = '\n*• قائمه العرر  \n ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉*\n'
for k, v in pairs(Info_Members) do
local UserInfo = bot.getUser(v)
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"• ماكو هناقروده حبيبي😂🗿, ","md",true)  
end
ListMembers = '\n*• قائمه القرود  \n ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉*\n'
for k, v in pairs(Info_Members) do
local UserInfo = bot.getUser(v)
...
if #Info_Members == 0 then
return send(msg_chat_id,msg_id,"• ماكو هنا اغبياء يروحي😂🙂 , ","md",true)  
end
ListMembers = '\n*• قائمه الاغبيه  \n ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉*\n'
for k, v in pairs(Info_Members) do
local UserInfo = bot.getUser(v)
...
local StatusMember = bot.getChatMember(msg_chat_id,senderr).status.Fastbots
if (StatusMember == "chatMemberStatusCreator") then
StatusMemberChat = 'مالك الگروب'
elseif (StatusMember == "chatMemberStatusAdministrator") then
StatusMemberChat = 'مشرف الگروب'
else
StatusMemberChat = 'عضو في الگروب'
end
...
if StatusMemberChat == 'مشرف الگروب' then 
